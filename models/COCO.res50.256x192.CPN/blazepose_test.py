...
from dataset import Preprocessing,PreprocessingV2
        ...
        while img_start < det_range[1]:
            ...
            for test_id in range(0, len(test_data), batch_size):
                ...
                for i in range(start_id, end_id):
                    test_img,o_img,roi ,img_name= PreprocessingV2(test_data[i], stage='test') #[1,3,256,256]
                    test_imgs.append(test_img) 
                    ori_imgs.append(o_img)
                    rois.append(roi)  # 
                ...
                for test_image_id in range(start_id, end_id): # for one img
                    original_img=test_imgs[test_image_id - start_id][0].transpose(1, 2, 0)*255 + cfg.pixel_means
                    whole_img=ori_imgs[test_image_id - start_id]
                    roi=rois[test_image_id - start_id].astype(np.int32)
                    img_name=imgnames[test_image_id - start_id]
                    #
                    ...
                    # normalize joints
                    cls_skeleton[:,:,0] /=cfg.data_shape[1]  #  x
                    cls_skeleton[:,:,1] /=cfg.data_shape[0]  #  y

                    # project joints into original image
                    cls_skeleton[test_image_id,:,:-1]=landmarkProjection(cls_skeleton[test_image_id,:,:-1],roi[:4],roi[-1])

                    if cfg.test_vis:
                        # image = np.ascontiguousarray(original_img, dtype=np.uint8) 
                        vis_img=whole_img.copy()
